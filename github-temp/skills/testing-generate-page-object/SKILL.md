...
```text
1. Load Knowledge -> 2. Read Input -> 3. Inspect Live Page (MCP) or Read Source Code -> 4. Reuse Existing Artifacts and Model Page/Component Structure -> 5. Generate Output -> 6. Self Review
```

**Step 3 — DOM Inspection Strategy:**
...
3. Inspect live page DOM using Playwright MCP tools if available:
   - Use `browser_navigate` to open the target URL.
   - Use `browser_snapshot` to capture the rendered DOM and verify actual element structure, roles, labels, and IDs.
   - Cross-validate proposed locators against the live snapshot before committing to code.
   - If Playwright MCP is unavailable, fall back to reading frontend source code (e.g., component `.tsx`/`.html` files) to infer locators.
4. Search and reuse existing page objects/components before creating new files.
5. Generate business-oriented methods and stable locators according to standards.
6. Keep assertions/test orchestration out of page object/component files.
...
3. If Playwright MCP tools are available, always use `browser_navigate` + `browser_snapshot` to verify actual DOM structure and locators before generating code. Do not rely solely on source code inference when live inspection is possible.
4. If Playwright MCP tools are unavailable, fall back to reading frontend source files (`.tsx`, `.html`, etc.) to infer DOM structure and locator candidates. Document the evidence source in the page object file header.
